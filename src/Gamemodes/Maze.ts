...
export default class MazeArena extends ArenaEntity {
  ...
  public constructor(game: GameServer) {
    ...
    private _buildWallFromGridCoord(gridX: number, gridY: number, gridW: number, gridH: number) {
        ...
        new MazeWall(this.game, scaledX, scaledY, scaledW, scaledH);
    }
    ...
    /** Checks if the point (x, y) is inside the arena and accessible in the maze */
    public contains(x: number, y: number): boolean {
        const halfSize = ARENA_SIZE / 2;

        // First check if (x,y) is inside arena bounds
        if (x < -halfSize || x > halfSize || y < -halfSize || y > halfSize) return false;

        // Convert world coordinates to grid coordinates
        const gridX = Math.floor((x + halfSize) / CELL_SIZE);
        const gridY = Math.floor((y + halfSize) / CELL_SIZE);

        // Check if grid coordinates are valid
        if (gridX < 0 || gridY < 0 || gridX >= GRID_SIZE || gridY >= GRID_SIZE) return false;

        // Return true if the maze cell is accessible (value 2), false otherwise
        return this._get(gridX, gridY) === 2;
    }

    /** New spawn system patch added here */
    public spawn(entity: { position: { set(x: number, y: number): void }}): void {
        entity.position.set(0, 0);
    }
}

export const maze = new MazeArena(null);
